...
DROP INDEX IF EXISTS wsrecordingmb_idx; 
CREATE UNIQUE INDEX wsrecordingmb_idx ON ws.recording_mb (id);
...
-----------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------
ALTER TABLE ws.recording_mb DROP COLUMN IF EXISTS mbid;
ALTER TABLE ws.recording_mb ADD COLUMN mbid INT;
UPDATE ws.recording_mb p SET mbid=
       (SELECT b.id
	...
	INNER JOIN (SELECT c.id, c.gid 
        ...
        ON a.ogid=b.gid
        WHERE a.ogid=p.ogid AND a.id=p.id);
/*
UPDATE 236723
Time: 6221,191 ms (00:06,221)
*/
DROP INDEX IF EXISTS wsrecordingmb_mbidx; 
CREATE INDEX wsrecordingmb_mbidx ON ws.recording_mb (mbid);
...
SELECT COUNT(DISTINCT mbid) FROM ws.recording_mb;
/*
 count  
--------
 232393
...
Time: 194,191 ms
So, there are ogid that have the same mbid (expected, because of the use of musicbrainz.recording_gid_redirect)
or that are not found 
*/

SELECT COUNT(DISTINCT id) FROM ws.recording_mb;
/*
 count  
--------
 236686
...
SELECT a.id, b.name, a.ogid FROM ws.recording_mb a INNER JOIN  ws.recording b ON a.id=b.id WHERE a.mbid IS NULL;
/*
   id   |                  name                   |                 ogid                 
--------+-----------------------------------------+--------------------------------------
...
DELETE FROM ws.recording_mb a
  WHERE a.id IN (SELECT b.id FROM ws.recording_mb b WHERE b.mbid IS NULL);
/*
DELETE 37
Time: 6,790 ms
*/
